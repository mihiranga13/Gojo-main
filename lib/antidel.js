...
async function initializeAntiDeleteSettings() {
    ...
    try {
        ...
        if (tableInfo.gc_status !== undefined) {
            ...
            if (oldRecord && oldRecord.length > 0) {
                const newStatus = oldRecord[0].gc_status || oldRecord[0].dm_status || false;

                // Dropping table is destructive - consider safer migration in real apps
                await DATABASE.query('DROP TABLE antidelete');
                await AntiDelDB.sync();
                await AntiDelDB.create({ id: 1, status: newStatus });
            }
        } else {
            // Create default record if not exist
        ...
        // If table not exists at all, create it
